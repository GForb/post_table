...
foreach k in `sub_grps'{
	...
	if `k' == `ref_sub_grp' {
		qui lincom `prim_treat'.`treat' 
		local p_int
	}
	else{
	*calculating p value for interaction
		qui lincom `prim_treat'.`treat'#`k'.`sub_var' 
	*p-value
		if "`tdist'" != "" {
				local p1 = 2*(1-t(`df', abs(r(estimate)/r(se))))
		
			if `p1' >=0.01 {
				local p_int = string(`p1', "%3.2f")
			}
			if `p1' >=0.001 & `p1' < 0.01 {
				local p_int = string(`p1', "%4.3f")
			}
			if `p1' < 0.001 {
				local p_int = "<0.001"
			}
			...
			local p1 = 2*(1-normal(abs(r(estimate)/r(se))))
		...
		lincom `prim_treat'.`treat'#`k'.`sub_var' + `prim_treat'.`treat', `eform'
		qui lincom `prim_treat'.`treat'#`k'.`sub_var' + `prim_treat'.`treat'
	}

	*calcualting estimate
	if "`tdist'" != "" {
		local coef = r(estimate)
		local ll = `exp1'(r(estimate) - invt(`df', 0.975)*r(se))`exp2'
		local ul = `exp1'(r(estimate) + invt(`df', 0.975)*r(se))`exp2'
		...
		local coef = `exp1'r(estimate)`exp2'
		local ll = `exp1'r(estimate) - invnormal(0.975)*r(se)`exp2'
		local ul = `exp1'r(estimate) + invnormal(0.975)*r(se)`exp2'
		local coef = string(`coef', "`form_est'")
		local ll = string(`ll', "`form_est'")
		local ul = string(`ul', "`form_est'")
...
*Other variable types
	...
	*creating entry in post file for summaries
	...
	post `postname' ("`sub_lab`k'' `measure_append' `append_sub_lab'") `measure_post' `inan_cols' `miss_cols' `summaries' ("`coef' (`ll', `ul')")  ("`p_int'") `post_icc'
