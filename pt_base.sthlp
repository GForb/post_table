...
{synopt:{opt t:ype(type)}}Specifies the summary statistics to be produced. {it:type} can be {cmd:cont}, {cmd:skew}, {cmd:bin}, {cmd:cat} or {cmd:misstable}. 
	If {cmd:type()} is not specified, then {it:type} is determined by the number of unique values the variable takes. A variable with10 or more unique values will be considered {cmd:type(cont)}. {p_end}
{synopt:{opt positive(#)}}Value that is considered positive for binary variables, default is {cmd:positive(1)}.{p_end}
...
{synopt:{opt cat_levels(numlist)}}Specify the order levels of a catagorical variable are to be reported{p_end}
{synopt:{opt cat_tabs(#)}}Change the indentation of the levels of a catagorical variable{p_end}
...
{synopt:{opt n:_analysis(postion cond %)}}Include number of nonmissing observations. {it:position} must be specified with {cmd:n_analysis()}, {cmd:cond} and {cmd:%} are optional.  {it:position} can be {cmd:cols}, {cmd:append}, or {cmd:brackets}. 
...
{synopt:{opt su_label(position)}}Position of the summary label. {it:position } can be {cmd:append } or {cmd:col}. Default is to omit summary label.{p_end}
...
All variables must be numeric; binary or categorical variable must have their values encoded as numbers. If variables and values are labelled the command used labels to name rows in the table by default.
...
The document {browse "https://github.com/GForb/post_table/tree/master/Examples/all_option_examples.do":pt_base_examples} includes code and the corresponding output for all options and can be used as a comprehensive guide.
